    ...
    <div className='h-full overflow-y-scroll p-6  flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={formSubmit} className='w-full max-w-lg p-4 bg-white border border-gray-200 rounded-lg'>
        <div className='flex items-center gap-3'>
          <Sparkles className='w-6v text-[#4A7AFF]' />
            ...
            {articleLength.map((item, index)=>(
              ...
              onClick={() => setSelectedLength(item)}
               className={`text-sm px-4 py-1 border rounded-full cursor-pointer 
                ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700' : 'bg-gray-300 text-gray-500'}`}
               key={index}>{item.text}
              </span>
            ))}
          </div> 
      ...
      min-h-96 max-h-[600px] flex flex-col'>
        <div className='flex items-center gap-3'>
        ...
        <div className='flex flex-1 justify-center items-center'>
          <div className='flex flex-col text-sm items-center gap-5 text-gray-400'>
            ...
            <p>Enter the topic and click "Get Started" to generate an article</p>
          </div>
        </div>
      </div>
    </div>
